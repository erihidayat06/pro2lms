            ...
            ->columns([
                ...
                TextColumn::make('progres')
                    ...
                    ->state(function (Model $record) : string {
                        ...
                        $jawaban_unfix = Jawaban::query()->where([['mahasiswa_id', $this->mahasiswa->id],['master_fix_id', null]])->get();
                ...
                $master_fix = MasterFix::query()->where([['master_soal_id',$record->id],['mahasiswa_id', $this->mahasiswa->id]])->get()->first();
                $total_bobot_nilai = Jawaban::where([['mahasiswa_id', $this->mahasiswa->id],['master_fix_id', $master_fix]])->sum('bobot_nilai');
                $jumlah_jawaban = Jawaban::where([['mahasiswa_id', $this->mahasiswa->id],['master_fix_id', $master_fix]])->count();
               if($master_fix){
                if($master_fix->is_correct){
                     return $total_bobot_nilai/$jumlah_jawaban; 
                }else{
